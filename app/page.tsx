...
const TOKEN_NAME = "Spiral Cast";
const TOKEN_TICKER = "SCT";
const TOTAL_SUPPLY = "500,000,000";
const CLAIM_DATE = "September 15";
...
const TIERS: Tier[] = [
  { key: "signal", label: "Signal", min: 0, max: 0.4, allocation: 2500, color: "#9C7A3F" },
  { key: "relay", label: "Relay", min: 0.4, max: 0.7, allocation: 7500, color: "#B8935A" },
  { key: "uplink", label: "Uplink", min: 0.7, max: 0.9, allocation: 17500, color: "#C9A227" },
  { key: "core", label: "Core Node", min: 0.9, max: 1.01, allocation: 37500, color: "#7A5C1E" },
];

type TokenomicsItem = { label: string; pct: number; color: string };

const TOKENOMICS: TokenomicsItem[] = [
  { label: "Airdrop", pct: 55, color: "#C9A227" },
  { label: "Liquidity", pct: 25, color: "#7A5C1E" },
  { label: "Team", pct: 10, color: "#B8935A" },
  { label: "Others", pct: 10, color: "#9C7A3F" },
];
    ...
    <main className="min-h-screen flex flex-col items-center px-5 py-10">
      <div className="w-full max-w-md flex flex-col items-center text-center mb-8">
        <div className="w-16 h-16 rounded-2xl bg-ink flex items-center justify-center mb-4 shadow-lg">
          <span className="font-display text-cream-50 text-2xl italic">S</span>
        </div>
      ...
      <div className="w-full max-w-md bg-cream-50 border border-ink/10 rounded-2xl shadow-sm p-6">
        {step === "loading" && (
          <p className="text-center font-body text-sm text-ink/50 py-6">Loading Spiral Cast…</p>
        )}

        {(step === "ready" || step === "connecting") && !address && (
          <div className="flex flex-col items-center gap-4 py-4">
            <p className="font-body text-sm text-ink/70 text-center">
              Connect your Farcaster wallet to check your Neynar Score and reveal your $SCT tier.
            </p>
            <button
      ...
      <div className="w-full max-w-md mt-6 bg-ink rounded-2xl shadow-sm p-5 flex items-center justify-between">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-widest text-cream-50/50">
            No Vesting · Direct Claim
          </p>
          <p className="font-display italic text-lg text-cream-50 mt-1">Claim Opens</p>
        </div>
      ...
      <div className="w-full max-w-md mt-6 bg-cream-50 border border-gold-400/40 rounded-2xl shadow-sm p-6">
        ...
        <p className="font-body text-sm text-ink/70 leading-relaxed">
          <span className="font-display italic text-ink/90">
            ${TOKEN_TICKER} launches on Base-native DEXs
          </span>{" "}
          — Aerodrome, Uniswap, and Curve Finance — with liquidity seeded from the treasury allocation.
        </p>
        <div className="flex justify-between items-center mt-4 pt-4 border-t border-ink/10">
          <span className="font-mono text-[11px] uppercase tracking-widest text-ink/50">
            CEX Listing
          </span>
          <span className="font-mono text-sm font-semibold text-ink">TBA</span>
        </div>
      </div>
